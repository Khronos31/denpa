...
保存先への配置までを行う。
出来上がった mkv は保存先に置かれ、ブラウザで観るか (`/watch`)、denpa が配って外部プレイヤー (テレビの VLC など) で見る。削除は手動。
...
| `src/lib/server/webhook.ts` | 録画の節目 (開始・完了・失敗)、エンコードの完了・失敗、そして録画の外側の変事 (ディスク残量わずか・チューナー切断/復帰) を外部へ通知する |
...
あって実際には録れる組にも印が付いていました。本数が分からないとき (エージェントに繋がらずに
いるとき) はどちらも何も言いません。

**丸ごと入らないからといって、丸ごと捨てません。** 取り合いは番組まるごとで
起きるとは限らない — 実機で出たのは、30分番組4本のうち **15分だけ**3チャンネル
要る形でした。空いている一番長い区間を見つけてそこだけ掴みます
(`assign` の「入るところまで録る」)。切られるのは**優先度の低いほう**です —
採るのが優先度の高い順なので、席が埋まったところへ来るのは必ず低いほう。
譲った区間は `record_from` / `record_to` に残り (番組の時刻は動かさない)、
予約一覧は「頭 15分 が欠けます」、録画一覧は「欠けています」と言います
(`format.clipNote`)。**マージンだけ削られたぶんは言いません** — 隣り合う番組は
マージンぶんが必ず重なるので、そこまで数えるとほとんどの録画に札が付きます。

**番組はマージンに勝ちます。** 22:00 終了 → 22:00 開始の隣り合いは、前の
終了マージンと後の番組そのものの取り合いです。マージンは「延びたときのための
保険」なので、番組に譲ります — 丸ごと競合にしていた頃は、**隣り合っている
というだけで後ろが録れません**でした。

録画中にプロセスが落ちた場合、次の起動でまだ放送中なら録り直します。生TSは追記で
開くので、落ちるまでに録れていた分は残ります。放送が終わっていれば失敗に倒します。

**掴んでいる者が居ないまま終了時刻を過ぎた録画は、見回りが畳みます** (`failStrayRecordings`、
猶予1分)。起動時の拾い直しだけでは足りません — そこで**録り直しに行った先でまた殺される**と
(ノードごと落ちる・Pod を強制終了する)、次の起動まで誰も畳まないためです。畳まれない行は
**予約の一覧に「録画中」で居座り、録画の一覧には出てきません** (どちらの振り分けも
`recordings.state = 'recording'` で決まる)。生TSは消さないので、録れていた分は詳細から焼き直せます。
...
| `RECONCILE_INTERVAL` | `300000` | 保存先の実体とDBを突き合わせる間隔(ms)。ディスク残量の見張りも同じ周期 |
| `DISK_LOW_THRESHOLD` | `21474836480` | 保存先の空きがこれ(バイト)を下回ったら通知する。既定は 20GB。`0` で見張らない |
| `AGENT_DOWN_GRACE` | `180000` | チューナー(エージェント)に繋がらない状態がこれ(ms)続いたら通知する。一瞬の切れでは鳴らさない |
...
| `BML_DNS` | `1.1.1.1,8.8.8.8` | データ放送の双方向で名前を引く DNS。**家庭の DNS フィルタが局のドメインを 0.0.0.0 に落とす**ことがあるので、素の DNS を名指しする。空なら OS の設定どおり |
...
| 画面 | 役割 |
| --- | --- |
| `/` | **予約と録画**を2ペインで並べる。**録画の行を押すと観る画面 (`/watch/<id>`) へ**、中身は「詳細」から (生TSがある間はエンコード中・エンコード失敗でも追っかけ再生 `/chase/<id>` へ。押せないのは録画失敗・削除済み・録り逃しで、その行は行そのものが詳細の入口)。めったに押さないもの (ダウンロード・再生リンク・再エンコード) は詳細の「その他…」に畳む。行の形はどの画面幅でも同じで、狭いところでは押すものが下へ回り込む。**行の頭にポスター** (`api/recordings/<id>/poster` = 焼いたときの `-poster.jpg`。無ければ枠だけ) で、テレビの画面でも文字だけより選びやすい。**途中まで観たものは「続き」の進捗バー** (`resume_ms` が残っている = まだ途中)。**置いてあるものは全部の大きさを出す** (`43 MB (H.264 71 MB、生TS 594 MB)`) — 主 (AV1) のぶんだけでは、消していいのか・どれだけ空くのかが読めない。**番組名・シリーズ・副題・局で絞り込める** (GET なので URL に残る)。**300件で頭打ちのときはその旨を出す** (古いものは絞り込みで探す)。**録り逃しは録画の一覧に出す** (「これから録るもの」ではなく録画の結果なので。以前は黙って消え、チューナーが足りずに落とされたものに気付けなかった)。確かめ終わったら2回押しで消せる。**どちらの行にも「何で立ったか」を出す** — ルール名がそのまま編集への入口で、録画の側は手動で録ったものもそう書く (ルールを消したあとは「ルール: (削除済み)」)。**エンコードの失敗ではダウンロードも録り直しも消さない** — 落ちたのは焼き直しのほうで、生TSは無事 (ただし**再生の印は出さない** — 生TSは MPEG-2 でブラウザが読めない) |
...
| `/live` | **いま流れているものを見る** ([stream.md](stream.md))。左に映像、右にチャンネル。**前回見ていた局から開く** (覚えていなければリモコン番号のいちばん若い局。番組表の「視聴」から来たときはその局)。局の並びは番組表と同じなので、あちらで見つけた局をここでも同じ位置で探せる。**操作列は自前** (備え付けは使わない) で、止めた所から見られる (**追いかける速さを選べ、追いついたら自分でライブに戻る**)・音声を選べる・**焼き方 (H.264 / AV1) を見ながら選べる**・**字幕を出せる**・**いまの1コマを字幕ごと切り抜ける** (観る画面と同じ)・切り替えの間は前の絵を貼る。**d と切り抜きは右上の縦列** (観る画面と同じ場所・同じ並び)。**右上の録画ボタン**を押すと、いま観ている番組の録画がそのまま始まる (手動予約と同じ道)。放送そのまま (MPEG-2) は出せない — ブラウザに復号器が無い ([stream.md](stream.md) §5.5)。**データ放送も出せる** (d ボタン。指で押せるリモコンが右に並ぶ)。**Hybridcast は行き先だけ出す** (別のタブで開く。動かさない) |
| `/chase/<id>` | **追っかけ再生** ([issue #16](https://github.com/danything/denpa/issues/16))。録画中の録画を頭から観る。一覧の**予約側**の録画中の行の「追っかけ」から (録れている最中の行はまだ予約の側に居る)。生TSはブラウザで読めないので、サーバが伸びるファイルを追い読みして**ライブと同じ器** (fMP4 → WebSocket → MSE) で焼き直す。チューナーは掴まない。**押し方は観る画面と同じ** (空白・矢印・画面内クリック、指は端2回で10秒)。速さも同じところに覚える。シークバーは番組の全長で、右端 (いま録れているところ) は伸びていく — 手元に残っている範囲は即座に、外は `chase` の送り直し (ffmpeg 立て直し) で跳ぶ。視聴位置は15秒おきにサーバへ写すので、焼き上がったあとの `/watch` が続きから拾う。焼き上がった録画で開くと `/watch` へ回す |
| `/watch/<id>` | **録画をブラウザで観る** ([library.md](library.md#観るのはブラウザで))。**タブレット以上は2段組で、左に映像・右に番組の中身** (ライブと同じ形・同じ幅 768px)、狭い画面では映像が上。指で触っている端末は**開いた時点で全画面**。**操作列は自前** — どこを押しても再生/一時停止 (指は操作列の出し入れ)、**左右の端を素早く2回で10秒** (送りのボタンは置いていない。PCは矢印キー)、**CMを自動で飛ばす** (既定で入。ロゴでの判定に失敗した録画だけ切って始める) とチャプター送り、**観たその場で削除**、**続きから再生**。閉じる・**データ放送 (d)**・切り抜き・削除は**右端の縦の列**（データ放送を持つ録画でだけ d を出す。押すと右にリモコン）。番組の中身は**右に出たまま** (モーダルにしない — 映像の上に被さると観ながら読めない)。**番組表から消えた録画でもジャンル・音声・詳細の札は出る** — 録り始めに `recordings` へ写してある分で、詳細の種はそれを既定の null で潰さない (`detail.svelte.ts`。潰していた頃は、1日過ぎた録画だけ札が消えていた)。観られるのは**焼いたものだけ** (`?source=encoded`)。**字幕は既定で出す** (ライブと同じ)。放送どおりの絵 (PGS) をそのまま重ねる。**倍速**も選べる (ライブの追っかけと同じ並び)。**音声トラックも選べる** — 二カ国語・二重音声を焼いたものは両方入っているので、ブラウザが器から出せたときだけ切り替えを出す (`video.audioTracks`。出せない端末では出さない。ライブと違いサーバは焼き直さない) |
...
| `/rules` | 自動予約ルールの一覧と作成。**優先度は予約どうしを比べる数** (0〜9。チューナーが足りないとき大きいほうを残す。チューナー画面の「掴む強さ」とは別の物差し)。「この条件で録れる番組」に**予約済みのものも競合も同じ表で**出す (1件ずつ取り消せる)。行を押すと予約一覧と同じ番組詳細が出て、そこから**その1本だけ予約**できる (条件を詰めている途中で「これは録っておきたい」に行き当たったとき、番組表を開いて同じ番組を探し直さずに済む。ルールに任せるかどうかとは別の話)。**いま流れていれば「視聴」**も出る (番組表の詳細と同じ判断 — `epg.watchableServices`)。**取り消しはここに出さない** — 行のほうに置いてあり、予約済みの行は状態の札になる。**「何が録れるか見る」は GET でこの画面に戻ってくる**ので、条件は全部 URL に乗る — 受け取る側が読み落とすと、見ただけのつもりが保存で書き換わる (優先度で実際に起きた) |
| `/tuners` | **チューナーの設定** (本数・デバイス・受けられる種別・LNB・無効化。書かなければ自動検出)、チャンネルスキャン (途中で中断でき、録画中でも実行できる)、チューナーの空きと何を掴んでいるか、取れているチャンネル (番組表の集まり具合つき)、エージェントとカードリーダーと局ロゴの状態 (**局ロゴをいますぐ取りに行く**: 地上波も衛星もチューナー2つで。進み具合が出る)。チューナーを掴んでいる相手は用途で出す (「録画: 番組名」「番組表 (T16)」「局ロゴ収集 (T16)」)。**CM検出用のロゴもここ** — 局ごとに覚えた絵を出し、位置を教える・捨てて覚え直す |
| `/settings` | 録画のしかた(映像コーデック — **AV1・H.264 を複数選べる**。両方選ぶと1本を2ファイルで焼く。どちらも外すと「エンコードしない」/CMの扱い/CMの探し方/ロゴをどれだけ当てにするか/コマ数を映像から決めるか/生TSを残すか/無料放送だけか)、**GPU** (見つかった口ごとに QSV / VA-API × コーデックの印。使えるものに自動で付く。「確かめ直す」で再探索)、通知先(Webhook)、データ放送(郵便番号・双方向の入切)、**テレビで再生 (VLC)** — 名前+IP+ポート+コーデックの行編集。登録が無いと詳細にボタンは出ない。コーデックは AV1 を解けないテレビに H.264 や生TSを渡すためのもの、EPGStation からの引き継ぎ、**画面の高さを見る** (端末ごと。PWA にはアドレスバーが無く `?measure` を打てないため、入り口をここに置いてある)。**広い画面では2列**で、幅いっぱいのカードは置かない (2列の下に帯が続くと、どこまでが左の列の続きか読めない) |
| `/api/recordings/<id>/file` | 録画ファイル。Range 対応。**エンコード済みがあればそちら、無ければ生TS。焼き直している間は生TS** (始める前に前のものを消すので、その間は保存先に何も無い)。`?source=ts` / `?source=encoded` / `?source=alt` (H.264) で**名指しもできる** — 残っているぶんだけ、詳細のダウンロードはこれで口を分けて出す ([library.md](library.md)) |
...
| 長い一覧 (どの画面でも) | **少しずつ出す** (`paging.svelte.ts`)。予約・録画・ルールは最初に 1 画面と少しだけ描き、下端に近づいたら足す — 数百行を一度に描くと開いた直後に一瞬止まって見えた。全部は描かないので Ctrl+F では探せず、**一覧ごとに絞り込みの欄**を置く (打った端から手元の行に当たる。録画は送ればサーバの絞り込みにもなる)。番組表のマスも同じ理由で数百個ずつ描く (描いている間は `guide-grid` に `aria-busy`)。**遷移が 150ms を超えたら上端に細いバー**を出す (`nav-progress`) — 押してから次の画面が届くまで何も変わらないのが「止まった」に見えていた |
